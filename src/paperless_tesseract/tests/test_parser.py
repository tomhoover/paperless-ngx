...
class TestParser(DirectoriesMixin, FileSystemAssertsMixin, TestCase):
    ...
    def test_get_text_from_pdf(self):
        ...
        text = parser.extract_text(
            None,
            self.SAMPLE_DIR / "simple-digital.pdf",
        )
    ...
    def test_get_dpi(self):
        ...
        dpi = parser.get_dpi(self.SAMPLE_DIR / "simple-no-dpi.png")
        self.assertEqual(dpi, None)

        dpi = parser.get_dpi(self.SAMPLE_DIR / "simple.png")
    ...
    def test_signed(self):
        ...
        parser.parse(self.SAMPLE_DIR / "signed.pdf", "application/pdf")
    ...
    def test_image_simple(self):
        ...
        parser.parse(self.SAMPLE_DIR / "simple.png", "image/png")
    ...
    def test_image_simple_alpha(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            sample_file = self.SAMPLE_DIR / "simple-alpha.png"
    ...
    def test_image_calc_a4_dpi(self):
        ...
        dpi = parser.calculate_a4_dpi(
            self.SAMPLE_DIR / "simple-no-dpi.png",
        )
    ...
    def test_image_no_dpi_default(self):
        ...
        parser.parse(self.SAMPLE_DIR / "simple-no-dpi.png", "image/png")
    ...
    def test_multi_page(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(
            self.SAMPLE_DIR / "multi-page-digital.pdf",
            "application/pdf",
        )
        self.assertIsFile(parser.archive_path)
        self.assertContainsStrings(
    ...
    def test_multi_page_analog_pages_skip(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(
            self.SAMPLE_DIR / "multi-page-images.pdf",
            "application/pdf",
        )
        self.assertIsFile(parser.archive_path)
        self.assertContainsStrings(
    ...
    def test_multi_page_mixed(self):
        ...
        parser = RasterisedDocumentParser(None)
        parser.parse(
            self.SAMPLE_DIR / "multi-page-mixed.pdf",
            "application/pdf",
        )
        self.assertIsNotNone(parser.archive_path)
        self.assertIsFile(parser.archive_path)
    ...
    def test_single_page_mixed(self):
        ...
        parser = RasterisedDocumentParser(None)
        parser.parse(
            self.SAMPLE_DIR / "single-page-mixed.pdf",
            "application/pdf",
        )
        self.assertIsNotNone(parser.archive_path)
        self.assertIsFile(parser.archive_path)
    ...
    def test_rotate(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(self.SAMPLE_DIR / "rotated.pdf", "application/pdf")
        self.assertContainsStrings(
            parser.get_text(),
            [
    ...
    def test_multi_page_tiff(self):
        ...
        parser = RasterisedDocumentParser(None)
        parser.parse(
            self.SAMPLE_DIR / "multi-page-images.tiff",
            "image/tiff",
        )
        self.assertIsFile(parser.archive_path)
        self.assertContainsStrings(
    ...
    def test_multi_page_tiff_alpha(self):
        ...
        """
        parser = RasterisedDocumentParser(None)
        sample_file = self.SAMPLE_DIR / "multi-page-images-alpha.tiff"
    ...
    def test_multi_page_tiff_alpha_srgb(self):
        ...
        """
        parser = RasterisedDocumentParser(None)
        sample_file = self.SAMPLE_DIR / "multi-page-images-alpha-rgb.tiff"
        with tempfile.NamedTemporaryFile() as tmp_file:
            shutil.copy(sample_file, tmp_file.name)
            parser.parse(
                tmp_file.name,
...
class TestParserFileTypes(DirectoriesMixin, FileSystemAssertsMixin, TestCase):
    ...
    def test_bmp(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(self.SAMPLE_DIR / "simple.bmp", "image/bmp")
        self.assertIsFile(parser.archive_path)
        self.assertIn("this is a test document", parser.get_text().lower())

    def test_jpg(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(self.SAMPLE_DIR / "simple.jpg", "image/jpeg")
        self.assertIsFile(parser.archive_path)
        self.assertIn("this is a test document", parser.get_text().lower())
    ...
    def test_gif(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(self.SAMPLE_DIR / "simple.gif", "image/gif")
        self.assertIsFile(parser.archive_path)
        self.assertIn("this is a test document", parser.get_text().lower())

    def test_tiff(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(self.SAMPLE_DIR / "simple.tif", "image/tiff")
        self.assertIsFile(parser.archive_path)
        self.assertIn("this is a test document", parser.get_text().lower())
    ...
    def test_webp(self):
        parser = RasterisedDocumentParser(None)
        parser.parse(self.SAMPLE_DIR / "document.webp", "image/webp")
        self.assertIsFile(parser.archive_path)
